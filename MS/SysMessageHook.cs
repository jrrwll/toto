...
namespace Toto.MS
{
    public class SysMessageHook
    {
        ...
        internal static extern int SetWindowsHookEx (HookType hookType, 
            HookProcedure lpfn, IntPtr handle, int dwThreadId);

        [DllImport("user32.dll", CallingConvention = CallingConvention.StdCall,
            CharSet = CharSet.Auto, SetLastError = true)]
        internal static extern int UnhookWindowsHookEx( IntPtr handle );
    ...
    /// </summary>
    public enum HookMessageType : uint
    {
        WM_KEYFIRST = 0x0100,
